...
@router.get("/random", response_model=QuestionResponse, summary="随机获取试题")
...
@router.get("/{question_id}", response_model=QuestionResponse, summary="获取试题详情")
...
@router.post("/", response_model=QuestionResponse, summary="创建试题")
async def create_question(
    ...
    question = await Question.create(**question_data.dict())
    return await Question.filter(id=question.id).prefetch_related(
        "semester", "grade", "subject", "category"
    ).first()


@router.put("/{question_id}", response_model=QuestionResponse, summary="更新试题")
async def update_question(
    ...
    return await Question.filter(id=question_id).prefetch_related(
        "semester", "grade", "subject", "category"
    ).first()
